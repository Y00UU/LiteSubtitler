...
import openai
from PyQt6.QtCore import pyqtSlot, QTime, QEventLoop, pyqtSignal
...
class LlmPrompterFacade(BaseObject):
    ...
    def __init__(self, func_write_log, config, icon: QIcon = None):
        ...
        self.translate_service = TranslateService(log_to_ui_func=func_write_log, callback=self.update_to_cell)  # 翻译服务
        ...
        if "models" in self.config_args:
            GuiTool.init_combobox(combobox=self.ui.cbbLlmModel, values={item: item for item in self.config_args["models"]})

        GuiTool.init_combobox(combobox=self.ui.cbbPrompt, values=self.config_args["PROMPT_FILES"], on_combobox_changed=self._load_prompt_)
        self._load_prompt_()

        GuiTool.init_combobox(combobox=self.ui.cbbTargetLanguage, values={val.value: val.name for val in SubtitleLanguageEnum})
    ...
    def _build_new_client_(self) -> openai.OpenAI:
        self.llm_changed = False
        return openai.OpenAI(base_url=self.ui.edtBaseUrl.text(), api_key=self.ui.edtAPiKey.text(), timeout=180)
    ...
    def _on_send_chat_msg_(self):
        ...
        if self.ui.ckbUsePrompt.isChecked() and prompt:
            chat_msgs.insert(0, {"role": "system", "content": prompt.replace("TargetLanguage", self.ui.cbbTargetLanguage.currentText())})
        if self.llm_changed:
            self.client = self._build_new_client_()
        if not self.chat_thread:
            # 启动新线程
            self.chat_thread = AIChatThread(ai_client=self.client, chat_msg=chat_msgs, model=self.ui.cbbLlmModel.currentText(), user_msg=user_input)
            ...
            self.chat_thread.init_client_msg(ai_client=self.client, chat_msg=chat_msgs, model=self.ui.cbbLlmModel.currentText(), user_msg=user_input)
    ...
    def _on_save_prompt_(self):
        FileUtils.write_text(file_path=self._read_prompt_file_path_(), text=self.ui.txtPrompt.toPlainText())

    def _on_save_as_prompt_(self):
        file_path = GuiTool.save_dialog(parent=self.dialog, filter_str="Text Files (*.txt)")
        if file_path:
            FileUtils.write_text(file_path=file_path, text=self.ui.txtPrompt.toPlainText())
    ...
    def _load_llm_models_async_ui(self, models: list):
        GuiTool().update_llm_models_list(models, cbb_llm_model=self.ui.cbbLlmModel, current_model=self.ui.cbbLlmModel.currentText())
    ...
    def _build_tableview_model_(self):
        ...
        return GuiTool.build_tv_model(tv=self.ui.tbvDemo, data=data, headers=headers, sizes=sizes)
    ...
    def _on_run_translate_(self) -> None:
        ...
        if self.asr_data:
            thread = AiTranslateThread(
                service=self.translate_service,
